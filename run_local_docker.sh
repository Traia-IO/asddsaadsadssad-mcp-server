...
# Script to build and run the asddsaadsadssad MCP Server locally in Docker
...
IMAGE_NAME="asddsaadsadssad-mcp-server"
CONTAINER_NAME="asddsaadsadssad-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running asddsaadsadssad MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# asddsaadsadssad MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ asddsaadsadssad MCP Server is running!${NC}"
echo
